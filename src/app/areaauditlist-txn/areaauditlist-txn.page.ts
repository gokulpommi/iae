...
import { LocateauditPage } from './../locateaudit/locateaudit.page';
...
@Component({
  selector: 'app-areaauditlist-txn',
  templateUrl: './areaauditlist-txn.page.html',
  styleUrls: ['./areaauditlist-txn.page.scss'],
})
export class AreaauditlistTxnPage implements OnInit {
  ...
  constructor( 
    ...
    this.jsonUrl = 'assets/json/areaauditlist_txn.json';
  ...
  async ionViewDidEnter(){
        ...
        // Get all elements
        const elements = document.querySelectorAll('#myForm *[name]');
       ...
       const elementsArray = Array.from(elements);
        for (const el of elementsArray) {
            ...
            if (id && element.tagName !== 'ION-CHECKBOX') {
        ...
        try {
          const data = await this.http.get('assets/json/frmbooking_txn.json').toPromise();
...
async updatePresetValuesForSAS(id: string, datatype: string, dataset: any,ctype:any) {
 ...
 async loadImages() {
    ...
    const folderPath = 'Download'; // Path to your folder
    ...
    try {
      ...
      if (folderExists) {
        // Read the directory
        const files = await this.file.listDir(this.file.externalRootDirectory, folderPath);
...
pointauditPage(){
    console.log('hello point audit page');
    this.navCtrl.navigateForward('/pointaudit');
}
async areaauditPage(){
    console.log('hello area audit page');

   
      //this.image_value['imagename'] = this.selectedImage;
      this.preset_value['imagename'] = this.selectedImage;

      const modal =await this.modalCtrl.create({
            component:LocateauditPage,          
            componentProps:{ 'userdetails':this.UserDetails,'modaltemplate':'goo'}
         });
   ...
   // this.navCtrl.navigateForward('/locateaudit',{state:this.preset_value});
}

downloadMap(){
    console.log('hello area audit page');
    this.navCtrl.navigateForward('/downloadmap');
