import { Component, OnInit,ViewChild,ChangeDetectorRef,Injector,ElementRef  } from '@angular/core';
...
@Component({
  selector: 'app-tracktimeline',
  templateUrl: './tracktimeline.page.html',
  styleUrls: ['./tracktimeline.page.scss'],
})
export class TracktimelinePage implements OnInit {
  ...
  @ViewChild(MapInfoWindow) infoWindow!: MapInfoWindow;
    ...
    public Options               : any          =  { scales: { yAxes: [{ticks: {beginAtZero:true }}]} };
  ...
  constructor(
        ...
        this.rptName = 'locationtracking_rpt';
  ...
  // Load locations from the tracking service
  loadLocations() {
   
      
      if (this.locations.length > 0) {
        // Set map's initial center to the last recorded location
        this.center = { lat: this.locations[this.locations.length - 1].latitude, lng: this.locations[this.locations.length - 1].longitude };

        // Prepare the polyline path for the timeline
        this.polylinePath = this.locations.map((loc) => ({
          lat: loc.latitude,
          lng: loc.longitude,
        }));
      }

      this.calculateTotalKilometersAndTime();
    }



  calculateTotalKilometersAndTime() {
      let totalDistance = 0;
      let startTime = this.locations[0]?.time;
      let endTime = this.locations[this.locations.length - 1]?.time;

      for (let i = 0; i < this.locations.length - 1; i++) {
        const loc1 = this.locations[i];
        const loc2 = this.locations[i + 1];

        totalDistance += this.calculateDistance(loc1.latitude, loc1.longitude, loc2.latitude, loc2.longitude);
      }

      this.totalKilometers = totalDistance;

      if (startTime && endTime) {
        this.totalTime = endTime - startTime; 
        const totalSeconds = this.totalTime / 1000;
        const hours = Math.floor(totalSeconds / 3600);
        const minutes = Math.floor((totalSeconds % 3600) / 60);
        const seconds = Math.floor(totalSeconds % 60);

      // Format time as 'hours:minutes:seconds'
      this.totalTimeFormatted = `${hours} hours ${minutes} minutes ${seconds} seconds`;
  
      }

      console.log(this.totalKilometers,this.totalTimeFormatted);
  ...
  onMarkerClick(marker: any, location: any) {
  ...
  ionViewDidEnter() {
    ...
    this.jsonUrl = 'assets/json/locationtracking_rpt.json';
        //this.http.get('assets/json/farmerapp_rpt.json').map(res => res.json()).subscribe(data => {
          this.http.get('assets/json/locationtracking_rpt.json', {responseType: 'json'}).subscribe((data:any) => {
           console.log(data);
            this.subtitles = data;
        ...
        // }
            ...
            this.mycmservice.sendReportParams(this.subtitles["rt"],inputValue, projectname, reportid, multiselectvalues, groupfield, columnproperty, stl,multitenant,this.reportParams).subscribe((responseData:any) => {
               ...
               this.events.loadDismiss();
                ...
                else if(this.subtitles["rt"] == "trackingreport"){
                    this.renderTrackingReport();
                }
                else{
                    this.renderTableReport(datas,this.subtitles);
                }
