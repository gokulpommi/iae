...
@Component({
  selector: 'app-sprayingactivity-rpt',
  templateUrl: './sprayingactivity-rpt.page.html',
  styleUrls: ['./sprayingactivity-rpt.page.scss'],
})
export class SprayingactivityRptPage implements OnInit {
  ...
  constructor( 
        ...
        this.rptName = 'sprayingactivity_rpt';
  ...
  ngOnInit() {  }
   ...
   ionViewDidEnter() {
    ...
    this.jsonUrl = 'assets/json/sprayingactivity_rpt.json';
        //this.http.get('assets/json/farmerapp_rpt.json').map(res => res.json()).subscribe(data => {
          this.http.get('assets/json/sprayingactivity_rpt.json', {responseType: 'json'}).subscribe((data:any) => {
           console.log(data);
            this.subtitles = data;
  ...
  async renderDisplayReport(){
        ...
        let imageid=this.names; 
              ...
              let imageJson:Record<string,any> ={};
          
                          imageJson['objid']=imageID;
                          imageJson['eid']="griduploadcollection";
                          imageJson['type']="image";
                           let stringify = JSON.stringify(imageJson);
                           let params = encodeURIComponent(stringify);
                        
                            let response = await this.http.get(this.singleton.resturl+'mobileserviceapi/getImageCloud?viewJson='+params,{responseType: 'json'}).toPromise();
                          console.log(response);
                          let imagedata = response as any;
                          if(imagedata != null)
                          {
                            this.modifyImage=true;
                            this.mImage='data:image/jpeg;base64,'+imagedata['fileContent'];
                          }
  ...
  renderTableReport(datas:any,report_json:any){
                        ...
                        if(rowdata[j] == "blockheaderrow" || rowdata[j] == "subtotalrow"|| 
                            ...
                            if((thead["children"][j] as HTMLElement).dataset['type']== "number" && cellVal != ""){
                              ...
                              if(this.isNumber(cellVal)){
                                const decimals = Number(noOfDecimalVal); 

                                
                                if (decimals > 0) {
                                    cellVal = cellVal.toFixed(decimals);
                                } else {
    ...
    reportButtonOnClick(event:any,index:any){
        ...
        for(let i=0;i<reportExpsArray.length;i++){
            ...
            if(reportExp.includes("viewtxn")){
                ...
                if(paramsArray[1] == "N"){
            ...
            else if(reportExp.includes("gsmCall")){
                 let mobile = reportExp.substring(reportExp.lastIndexOf("(") + 1, reportExp.lastIndexOf(")"))
                 let ind = this.titles.indexOf(mobile);
                 ...
                 if(MobileNumber!=null){
                     ...
                     let gsmcall=this.callNumber.callNumber(MobileNumber, true).then((res:any) => {
                ...
                if(configJSonArray.length == 1){
                    ...
                    for (let i=0;i<this.otherService.pages.length;i++){
                        if(this.otherService.pages[i]['id'] == configJSon.tt){
                            txnpage = this.otherService.pages[i]['id'];        
                        }        
                    }
                    this.UserDetails['onthefly'] = true;
                    ...
                    this.navCtrl.navigateForward('/'+txnpage,{state:this.UserDetails});
    ...
    displayReportbtn(event:any,btnid:any,index:any){
    console.log(event);
    const btnFilter = this.subtitles.reportfield_meta.filter((item:any)=>{
              return (item['sl'] == btnid);
                        });
    if(btnFilter.length >0){
      event.currentTarget['dataset']['exp'] = btnFilter[0]['exp'];
      if(btnFilter[0]['exp'] == "print()"){
        
         //this.printReport(event,this.fab)
      }
      else{       
       this.reportButtonOnClick(event,index)
      }
    }
