...
@Component({
  selector: 'app-farmergeotagging-txn',
  templateUrl: './farmergeotagging-txn.page.html',
  styleUrls: ['./farmergeotagging-txn.page.scss'],
})
export class FarmergeotaggingTxnPage implements OnInit {
  ...
  constructor( 
    ...
    this.jsonUrl = 'assets/json/farmergeotagging_txn.json';
  ...
  async ionViewDidEnter(){
        ...
        try {
          const data = await this.http.get('assets/json/farmergeotagging_txn.json').toPromise();
...
getlocation(){
   ...
   this.statimageUrl = this.generateImageUrl(this.latitude, this.longitude);

 try {
    const blob = await this.http.get(this.statimageUrl, { responseType: 'blob' }).toPromise();
    const base64Data = await this.convertBlobToBase64(blob!);
    this.base64Image = base64Data;  // Set base64 data as the image source
      let obj:Record<any,any> = {};
      obj['uploadstatus'] = false; 
      obj['filename'] = this.latitude+','+this.longitude+'.png';                    
      obj['mediaType']="image/"+"png";
      obj['uploadstatus'] = false;      
      obj['imagefromWhere'] = "taken";
      obj['imageData'] = this.base64Image;
      this.image_meta['geotag'] = obj;
 ...
 nativeGeocoderOptions: NativeGeocoderOptions = {
   ...
   getAddress(lat:any,long:any){
    ...
    .then((res: NativeGeocoderResult[]) => {
      ...
      this.preset_value['frmgeoaddress']=this.address;
   ...
   generateImageUrl(lat: number, long: number): string {
    // Example: Using Google Static Maps API
    const apiKey = environment.apiKey;
    return `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${long}&zoom=15&size=600x300&maptype=roadmap&markers=color:red%7C${lat},${long}&key=${apiKey}`;
  }

 async uploadgps(imageid:any){
      ...
      const loading =await this.loadingCtrl.create({
        ...
        this.gpsupload(this.image_meta[imageid],this.preset_value,imageid,loading);

  }


   gpsupload(imagemeta:any,preset_value:any,id:any,loading:any){
